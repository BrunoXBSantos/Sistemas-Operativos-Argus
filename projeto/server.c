...
#define MAX_PIPES_COMANDO 500
#define MAX_PALAVRAS_PIPE 500
...
char *getPrimeiraPalavra();
char *getSegundaPalavra();
...
int inserirElementoLista();
int inserirElementoLista();
LLista criarElemento();
...
int main(int argc, char *argv[]){
	...
	if((fd_tarefasConcluidas = open("lista-tarefas-concluidas.txt", O_RDWR | O_CREAT | O_TRUNC, 0644))==-1){
		perror("Abertura da lista de tarefas concluidas");
		_exit(5);
	}
	...
	if((fd_fifo1=open(fifo1,O_RDONLY)) == -1){
		perror("Erro ao criar ficheiro");
		exit(2);
	}

	if((fd_fifo2=open(fifo2,O_WRONLY)) == -1){
		perror("Erro ao criar ficheiro");
		exit(2);
	}	
...
void comunicacao(char comandosRecebidos[MAX_CARACTERES_COMANDO]){
	...
	int numeroTarefaTerminar, pidTarefaTerminar;
	...
	if((strcmp(primeiraPalavra,"tempo-inatividade") == 0) || (strcmp(primeiraPalavra,"-i") == 0)){
		...
		if(pid<0){
			perror("Processo filho responsavel pela tarefa");
			_exit(3);
		}
...
void listar(){
	char temp[MAX_CARACTERES_COMANDO+31];
	LLista p=tarefasExecucao;
	if(p==NULL){
...
void historico(){
	char temp[1024]="";
	int n, fd;
	if((fd=open("lista-tarefas-concluidas.txt", O_RDONLY)) == -1){
		perror("Erro ao abrir o ficheiro das tarefas concluidas");
		_exit(4);
	}
...
int inserirElementoLista(int identificacaoTarefa, char tarefaAdicionar[MAX_CARACTERES_COMANDO], int pidTarefa){
